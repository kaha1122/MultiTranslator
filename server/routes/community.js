...
const NOTIF_KINDS = new Set([
    ...
    'lounge_reply', // 라운지 메시지의 답글(2026-09-11 추가) — 이동: /lounge/{dayId}?anchor=l:{mid}:{rid}
]);
router.post('/api/community/notify', requireAuthAny, rateLimit('community-notify', { perMinute: 60, perHour: 600 }), async (req, res) => {
    ...
    const { recipientUid, kind, postId, titleId, media, dayId, preview, actorName, actorPhoto, anchor } = req.body || {};
    ...
    try {
        const notif = {
            ...
            dayId: (typeof dayId === 'string' && /^\d{8}$/.test(dayId)) ? dayId : null, // 이동 대상: 라운지 날짜 방(lounge_threads doc id = YYYYMMDD)
            // 이동 앵커(r:{uid}|d:{cid}[:{rid}]|c:{cid}[:{rid}]|l:{mid}[:{rid}]) — 알림 탭 시 해당 댓글/평가로 스크롤(클라 notifRoute·푸시 notifUrl 공용).
            anchor: (typeof anchor === 'string' && /^[rdcl]:[A-Za-z0-9_-]{1,128}(:[A-Za-z0-9_-]{1,128})?$/.test(anchor)) ? anchor : null,
