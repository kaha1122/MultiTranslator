...
const SKIP_TITLES = {
  ...
  // 🚨 스핀오프 오귀속(2026-09-01 실사고 — dari-review 게시 체이닝에서 발생): "슬기로운 의사생활
  // N화 하이라이트" 검색 결과가 스핀오프 「언젠가는 슬기로울 전공의생활」(2025) 클립이고, 그 클립의
  // **설명문에 "\'슬기로운 의사생활\' 스핀오프 드라마"가 상시 포함**돼 설명문 귀속 경로(작품명 대조)를
  // 통과한다 — 채널(tvN DRAMA)·길이(9~10분)·회차 표기까지 전부 정상이라 다른 게이트로는 못 거른다.
  // E1~E12 전부 스핀오프 클립으로 오저장 → titles/235355/media/clips로 이전(사용자 지시, 회차 1:1).
  // 본편 요약본 실측(2026-09-01): E1·E6·E12·S2 검색 전부 스핀오프뿐 — 본편(2020~21) 공식 업로드는
  // 2~3분 장면 클립 포맷만 존재(64010 응팔과 동일한 구조적 공백), 디글 클래식에도 없음(예능만).
  96102: '스핀오프(235355) 클립이 설명문의 본편 언급으로 오귀속(2026-09-01 실사고·이전 완료) + 본편 회차 요약본 자체가 없음(실측 0건)',
};
