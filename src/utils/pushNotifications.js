...
// APNs raw hex(64자 대문자 hex)는 FCM token이 아님 — firebase-admin sendEachForMulticast가
// invalid-argument로 거부하므로 fcmTokens 배열에 절대 들어가면 안 됨.
// 정상 FCM token은 ~150자 이상, ':' 포함, 소문자/숫자/하이픈/언더스코어 혼재.
function looksLikeApnsHex(token) {
...
export async function saveFcmTokenToFirestore(uid, token) {
    ...
    // Fix A — 입구 sanity check: APNs raw hex가 어떤 경로로 들어와도 fcmTokens 배열에 저장 차단.
...
export async function registerIOSFCM(uid) {
    console.log('[Push-iOS] registerIOSFCM start:', { hasUid: !!uid });
